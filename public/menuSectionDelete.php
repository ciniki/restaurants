...
// -------
//
function ciniki_restaurants_menuSectionDelete(&$ciniki) {
    //
    ...
    $rc = ciniki_restaurants_checkAccess($ciniki, $args['tnid'], 'ciniki.restaurants.menuSectionDelete');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    if( !isset($rc['section']) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.restaurants.12', 'msg'=>'Section does not exist.'));
    }
    $section = $rc['section'];
    ...
    $rc = ciniki_core_objectCheckUsed($ciniki, $args['tnid'], 'ciniki.restaurants.menuSection', $args['section_id']);
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.restaurants.13', 'msg'=>'Unable to check if the section is still being used.', 'err'=>$rc['err']));
    }
    if( $rc['used'] != 'no' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.restaurants.14', 'msg'=>'The section is still in use. ' . $rc['msg']));
    }
    ...
    // Remove the section
    //
    $rc = ciniki_core_objectDelete($ciniki, $args['tnid'], 'ciniki.restaurants.menusection',
