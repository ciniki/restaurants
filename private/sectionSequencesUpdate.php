...
// Description
// ===========
// This function will update the sequences for restaurant menu section items.
//
// Arguments
// =========
// ciniki:
...
// <rsp stat="ok" />
//
function ciniki_restaurants_sectionSequencesUpdate($ciniki, $tnid, $section_id, $new_seq, $old_seq) {
    ...
    $strsql = "SELECT id, sequence AS number "
        ...
        . "WHERE section_id = '" . ciniki_core_dbQuote($ciniki, $section_id) . "' "
    ...
    if( isset($rc['rows']) ) {
        ...
        foreach($sequences as $sid => $seq) {
            ...
            if( $cur_number != $seq['number'] ) {
                $strsql = "UPDATE ciniki_restaurant_menu_items SET "
                ...
                ciniki_core_dbAddModuleHistory($ciniki, 'ciniki.restaurants', 'ciniki_restaurant_history', $tnid, 
                    2, 'ciniki_restaurant_menu_items', $seq['id'], 'sequence', $cur_number);
                $ciniki['syncqueue'][] = array('push'=>'ciniki.restaurants.menuitem', 'args'=>array('id'=>$seq['id']));
            }
            $cur_number++;
        }
    }
